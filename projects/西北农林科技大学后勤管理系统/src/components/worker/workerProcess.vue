      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span>{{scope.row.state===1? '申报' : (scope.row.state===2? '审核' : (scope.row.state===3? '出工':(scope.row.state===4?'修理':'完工')))}}</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="studentname"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
              @click="nextStep" :disabled="scope.row.state==='完工'">下一步</el-button>
            <el-button
              type="danger"
...
export default {
  ...
  methods: {
    nextStep () {
      if (this.tableData[0].state < 5) {
        this.tableData[0].state += 1
        this.$axios.put('/api/process/', this.tableData[0])
          .then(res => {
            console.log(res)
          })
          .catch(err => {
      ...
      } else if (this.tableData[0].state === 5) {
